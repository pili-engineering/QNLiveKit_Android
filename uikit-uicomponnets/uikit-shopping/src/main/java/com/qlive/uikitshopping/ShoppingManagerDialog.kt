...
class ShoppingManagerDialog(
    ...
    private fun setCount(filterStatus: Int, count: Int) {
        when (filterStatus) {
            -1 -> rbAll.text = "全部($count)"
            QItemStatus.ON_SALE.value -> rbOnSale.text = "上架中($count)"
            QItemStatus.PULLED.value -> rbPulled.text = "已下架($count)"
        }
    }
    ...
    inner class ShoppingManagerPage : SmartRecyclerView {
        ...
        inner class ShoppingManagerPageAdapter : BaseQuickAdapter<QItem, BaseViewHolder>(
